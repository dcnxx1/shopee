var stockArray = [
    ...
    },
    {
      ...
      "type": "jacket",
      "brand": "Gucci",
      "model": "GG Kaleidoscope Jacket Oranje",
      "sex": "male",
      "price": 1.695,
      "size": [
        "XS",
    ...
    },
    {
      ...
      "type": "pants",
      "brand": "Dior",
      "model": "Timmermansbroek | Dior Oblique Kasuri-jacquard",
      "sex": "male",
      "price": 1.35,
      "size": [
        "27x30",
    ...
    },
    {
      ...
      "type": "earring",
      "brand": "Gucci",
      "model": "Oorbellen bij met Gucci script",
      "sex": "female",
      "price": 550,
      "size": [
        ""
    ...
    },
    {
      ...
      "type": "earring",
      "brand": "Versace",
      "model": "Greca en Medua oorbellen",
      "sex": "female",
      "price": 290,
      "size": [
        ""
  ...
  const price_ascending = document.getElementById('price_ascending')
  const price_descending = document.getElementById('price_descending')
  ...
  const deleteButton = document.querySelectorAll('.dlt')
  
  deleteButton.forEach(element => deleteButtonsArray.push(element))
  
  var clickArray = [price_ascending, price_descending, shoe, skirt,jacket,pants,earring]
  
  
  
  
  function setFilterOptions() {
      clickArray.forEach(obj => {
          obj.addEventListener('click', (e) => {
      
          const attr = e.target.attributes.getNamedItem('data-trans').value
        const {id} = e.target
        const found = filterOptions.some(el => el.clickId == id)
               
        
        if(!found){
        if(id == 'price_ascending' && filterOptions.some(el => el.clickId == 'price_descending')){
        // if selected id from span is price ascending and price descending is inside filterOptions array, do nothing
        } else if(id == 'price_ascending' && !filterOptions.some(el => el.clickId == 'price_descending')){
        // if selected id form span is price_ascending and price_descending inside filterOptions does not exist, add price_ascending
        
        filterOptions.push({filterId: Math.random().toExponential(1), attrTrans: attr, clickId: id})
  ...
  function returnSelectedDeletedRequestItem(){
  var deletedRequestItem = ""
      deleteButtonsArray.forEach(element => {
          element.addEventListener('click', event => {
              const deletedRequestValue = event.target.attributes.getNamedItem('data-id').value
                  filterOptions = filterOptions.filter(obj => deletedRequestValue !== obj.clickId  )
  ...
  returnSelectedDeletedRequestItem()
  
  function handlePriceChange () { 
  return filterOptions.map(req =>{
   if(req.clickId == 'price_ascending'){
   const answ = stockArray.sort((a,b) => a.price - b.price)
  ...
  function filterItems(){ // Final
  const filteredPrice = handlePriceChange()
  var arrayResponse = []
  filterOptions.map(req => {
  arrayResponse.push(stockArray.filter(obj => obj.type == req.clickId))
  })
